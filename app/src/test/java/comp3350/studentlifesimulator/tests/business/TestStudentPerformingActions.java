...
public class TestStudentPerformingActions extends TestCase {
    ...
    public void testPerformingAction() {
        ...
        boolean result = StudentPerformingActions.makeStudentPerformAction(student, possibleAction, time);
        assertTrue(result);
        ...
        result = StudentPerformingActions.makeStudentPerformAction(student, anotherPossibleAction, time);
        assertTrue(result);
        ...
        assertFalse(StudentPerformingActions.makeStudentPerformAction(student, impossibleAction, time));
        assertEquals(Student.getMaxEnergy() + possibleAction.getEnergyUnit() + anotherPossibleAction.getEnergyUnit(), student.getCurrentEnergy());

        Action anotherImpossibleAction = new Action("Action that adds too much to the student's remaining energy", 10, 1, 0);
        assertFalse(StudentPerformingActions.makeStudentPerformAction(student, anotherImpossibleAction, time));
        assertEquals(Student.getMaxEnergy() + possibleAction.getEnergyUnit() + anotherPossibleAction.getEnergyUnit(), student.getCurrentEnergy());
